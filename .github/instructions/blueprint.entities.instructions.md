...
- **`entity/`** - Base entity classes (inherit `SnotelEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, SnotelEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import SnotelEntity`
