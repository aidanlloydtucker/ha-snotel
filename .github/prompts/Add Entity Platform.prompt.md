...
**Directory:** `custom_components/snotel/[platform]/`
...
from .entity import SnotelEntity
from .[entity_file] import Snotel[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: SnotelDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            Snotel[EntityName](coordinator, entry),
...
Add platform to `custom_components/snotel/manifest.json`:
...
- [ ] Entity class inherits from both `SnotelEntity` and platform class
...
- **Domain:** `snotel`
- **Class prefix:** `Snotel`
- **Base entity:** `SnotelEntity` in `entity/base.py`
- **Coordinator:** `SnotelDataUpdateCoordinator`
