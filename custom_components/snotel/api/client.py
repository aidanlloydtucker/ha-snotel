...
class SnotelApiClientCommunicationError(
    SnotelApiClientError,
):
    """Exception to indicate a communication error with the API."""


class SnotelApiClientAuthenticationError(
    SnotelApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        SnotelApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise SnotelApiClientAuthenticationError(
            msg,
        )
...
class SnotelApiClient:
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            SnotelApiClientAuthenticationError: If authentication fails.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise SnotelApiClientCommunicationError(
                msg,
            ) from exception
