...
- **Domain:** `snotel`
- **Title:** Snotel
- **Repository:** aidanlloydtucker/ha-snotel

**Key directories:**

- `custom_components/snotel/` - Main integration code
...
- Integration logs: `custom_components.snotel: debug` in `config/configuration.yaml`
...
- Use the domain `snotel` for all DOMAIN references
- Prefix all integration-specific classes with `Snotel`
- Use "Snotel" as the display title
...
- Inherit from platform base + `SnotelEntity`
...
- `tests/` mirrors `custom_components/snotel/` structure
