...
This document describes the technical architecture of the Snotel custom component for Home Assistant.
...
│   ├── __init__.py          # Exports SnotelDataUpdateCoordinator
...
│   ├── __init__.py          # Exports SnotelEntity
...
- `base.py` - Main coordinator class (`SnotelDataUpdateCoordinator`)
...
**Key class:** `SnotelDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `SnotelConfigFlowHandler` (main flow)
- `SnotelOptionsFlow` (options)
...
**Key class:** `SnotelEntity` (in `entity/base.py`)
...
2. `SnotelEntity` for common functionality
...
1. Create directory: `custom_components/snotel/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `SnotelEntity`
